...
def ensure_converted(
    ...
    *,
    ...
    metadata_path = cache_root / (item["blimp_model_name"] + ".source.json")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--full-dev-loss",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "eval/full_dev_loss.py",
    )
    parser.add_argument(
        "--converter",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "eval/convert_nanogpt_to_hf.py",
    )
