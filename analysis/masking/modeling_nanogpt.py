...
ATTN_RES_MASK_MODES = {"none", "nonrecent", "embed", "random_count_matched"}
...
def build_attn_res_keep_mask(
    *,
    ...
    if router_site == "q1" and block_start:
        ...
        # `blocks` is age >= 2.
        nonrecent_indices = completed_indices
    else:
        # The last completed block is age 1; all earlier ones are nonrecent.
        nonrecent_indices = completed_indices[:-1]

    if mode == "nonrecent":
        removed = nonrecent_indices
    else:
        ...
        removed = random.Random(site_seed).sample(
            completed_indices, k=len(nonrecent_indices)
        )
