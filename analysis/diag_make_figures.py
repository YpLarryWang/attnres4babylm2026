...
MASK_MODES = ("nonrecent", "embed", "random_count_matched")
...
def masking_contrasts(behavior_rows: list[dict]) -> list[dict]:
        ...
        individual.append(
            {
                ...
                "cost_nonrecent_pp": costs["nonrecent"],
                "cost_embed_pp": costs["embed"],
                "cost_random_count_matched_pp": costs["random_count_matched"],
                "nonrecent_excess_vs_random_pp": costs["nonrecent"] - costs["random_count_matched"],
            }
        )
    grouped = {}
    ...
    means = []
    value_fields = (
        "attnres_gain_pp", "cost_nonrecent_pp", "cost_embed_pp",
        "cost_random_count_matched_pp", "nonrecent_excess_vs_random_pp",
    )
...
def render_figure_b(mask_rows: list[dict], selected: set[str], output: Path) -> None:
    ...
    old = np.array([float(row["cost_nonrecent_pp"]) for row in terms])
    ...
    axis.scatter(x, old, color="#087e8b", s=42, label="mask nonrecent (age ≥ 2)")
...
def main() -> None:
    ...
    write_csv_exclusive(output_dir / "diag_masking_contrasts.csv", mask_rows, (
        "seed", "task", "level", "term", "attnres_gain_pp", "cost_nonrecent_pp",
        "cost_embed_pp", "cost_random_count_matched_pp", "nonrecent_excess_vs_random_pp",
        "selected_main",
    ))
