...
def main():
    ...
    ap.add_argument("--provenance-output", help="optional local provenance JSON; keep outside the exported model directory")
    args = ap.parse_args()
    if args.provenance_output:
        model_dir = os.path.realpath(args.out)
        metadata_file = os.path.realpath(args.provenance_output)
        if os.path.commonpath([model_dir, metadata_file]) == model_dir:
            ap.error("--provenance-output must be outside --out")
    ...
    # Match the tokenizer used in training, including space-prefixed word tokens.
    ...
    # Remove provenance files left by an older export into this directory.
    for filename in ("checkpoint_source.json", "parity-check.txt"):
